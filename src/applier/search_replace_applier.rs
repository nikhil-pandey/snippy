...
impl Applier for SearchReplaceApplier {
    async fn apply(&self, block: &ParsedBlock) -> Result<(), ClipboardError> {
        ...
        let search_replace_re = Regex::new(
            r"(?s)<<<+\s*SEARCH\r?\n(.*?)===+[^\r\n]*\r?\n(.*?)>>>+\s*REPLACE"
        ...
        for cap in search_replace_re.captures_iter(&block_content) {
            ...
            debug!("Search: '{}'\nReplace: '{}'", search_content, replace_content);
            ...
            if search_content.trim().is_empty() {
                ...
                if current_content.contains(search_content_without_newline) {
                    debug!("Found search content without newline in file: '{}'", search_content_without_newline);
                    ...
                    error!("Failed to find content to replace in file {}: '{}'", path.display(), search_content);
                }
            }
        }

        if successful_replacements == 0 {
            return Err(ClipboardError::ContentApplicationError(
                format!("Failed to find any content to replace in file {}", path.display())
            ));
        }

        if current_content.trim().is_empty() {
            remove_file_async(&path).await.map_err(|e| {
                ClipboardError::IoError(e.to_string())
            })?;
            info!("Deleted file {}", path.display());
        } else {
            write_file_async(&path, &current_content).await.map_err(|e| {
                ClipboardError::IoError(e.to_string())
            })?;
